...
import getImpactData from '@salesforce/apex/ImpactAnalysis.getImpactData';
export default class ImpactAnalysis extends LightningElement {
  @api recordId;
  @api objectApiName;
  @track errors;
  @api ImpactData;
  ...
  @wire(getImpactData,{recordId:'$recordId'}) traceData
  ({ error, data }) {
  if (data) {
      console.log(data);
...
initializeD3() {
  let treeData, data,
  path, selected = null,
  marginWidth = 5;// adjusting the X Position of tree
//parsing the String 
treeData = JSON.parse(this.ImpactData);
...
for (let i = 0; i < children.length; i++) {
  imghtIncrement = imghtIncrement + 500;
}
//Predefined Box Size 
let rectNode = {
  width: 120,
...
function update(source) {
  ...
  rectGrpEnter.append('rect')
    ...
    .on("contextmenu", function (d) { contextMenu(d); });
...
function contextMenu(d, openCallback) {
    ...
    },
    {
      ...
      action: function (elm, i) {
        if (!d.children) {
          ...
          let myObj, x = "";
          let id = d.data.id;
          getTraceabilityData({ recordId: id }).then((res) => {
            myObj = res;
            let children = JSON.parse(myObj);
            children = children.children;
            for (let i in children) {
              let newNodeObj = children[i];
              //Creates new Node
  ...
  function contextMenuEvent() {
    ...
    let elm, openCallback, index = this;
